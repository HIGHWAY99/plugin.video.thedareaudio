...
def Menu_LoadCategories(section=_default_section_): #Categories
	...
	### ###################################################################################################################################################################################################################################
	### ###################################################################################################################################################################################################################################
	if ( section == ps('section.radio')):
		_addon.add_directory({'section': section, 'mode': 'RadioCategories', 'url': '' },	 				{'title':  cFL('R',ps('cFL_color'))+'adio Categories'},fanart=_artFanart,img=art('genre','.jpg'))
		#
	elif  ( section == ps('section.trailers')): ## Trailers #################################################################################################################################################################################
...
def Menu_MainMenu(): #The Main Menu
	...
	_addon.add_directory({'mode': 'LoadCategories', 'section': ps('section.radio')},		{'title':  cFL('R',ps('cFL_color'))+'adio Stations'}  ,img='http://www.thedarewall.com/mp3/images/bgcont.png'				,fanart=_artFanart)
...
def RadioPlay(section,url):
	play=xbmc.Player(xbmc.PLAYER_CORE_AUTO) ### xbmc.PLAYER_CORE_AUTO | xbmc.PLAYER_CORE_DVDPLAYER | xbmc.PLAYER_CORE_MPLAYER | xbmc.PLAYER_CORE_PAPLAYER
	play.play(url)
	_addon.resolve_url(url)
	

def RadioStations(section,url):
	#prefix='http://www.thedarewall.com/mp3/radio.php'
	WhereAmI('@ Radio:  Categories -- url: %s' % url) 
	html=net.http_GET(url).content
	#html=messupText(html,_html=True,_ende=True,_a=False,Slashes=False)
	deb('html length',str(len(html)))
	### document.getElementById("fbar").innerHTML = '<div style="margin-top: 10px;border-radius: 6px;-moz-border-radius: 6px;width: 250px;padding: 5px;background: #111;border: 2px solid #212121;"><span id="radiop"></span>&nbsp;MTH house</div>';
	### var so = new SWFObject('http://www.thedarewall.com/mp3/flash/emff_silk_button.swf','ply','16','16','9','#000000');
	###   so.addParam('allowfullscreen','true');
	###   so.addParam('allowscriptaccess','always');
	###   so.addParam('wmode','transparent');
	###   so.addVariable('src','http://house.mthn.net:8500/;Ompthreeapi.mp3');
	###
	s='>\s*<span\s+id="radiop">\s*</span>\s*&nbsp;\s*(.+?)</div>\';[\n]'
	s+="\s*var\s+so\s+=\s+new\s+SWFObject\('http.+?.swf','\D+','\d+','\d+','\d+','#\d+'\);[\n]"
	s+="\s*so.addParam\('.+?'\s*,\s*'.+?'\);[\n]"
	s+="\s*so.addParam\('.+?'\s*,\s*'.+?'\);[\n]"
	s+="\s*so.addParam\('.+?'\s*,\s*'.+?'\);[\n]"
	s+="\s*so.addVariable\('src'\s*,\s*'(.+?)'\)"
	#s=''
	matches=re.compile(s).findall(html)
	...
	for name, path in matches:
		pars={'section':section, 'mode': 'RadioPlay', 'url': path }
		title=''+name+''
		labs={'title': title }
		_addon.add_directory(pars, labs, total_items=ItemCount)
		#_addon.add_directory(pars, labs, img=img, fanart=_artFanart, total_items=ItemCount)
	set_view('list',addst('default-view')); eod()



def RadioCategories(section):
	prefix='http://www.thedarewall.com/mp3/radio.php'
	url='http://www.thedarewall.com/mp3/radio.php'
	WhereAmI('@ Radio:  Categories -- url: %s' % url) 
	html=net.http_GET(url).content
	#html=messupText(html,_html=True,_ende=True,_a=False,Slashes=False)
	deb('html length',str(len(html)))
	### <div id="tabtop" style="background: #282828;"><a href="?genre=Hiphop/Rnb">HipHop/Rnb</a></div>
	#s='<div id="tabtop" style="background: #282828;"><a href="(.+?)">(.+?)</a></div>' ## works for some
	s='<div\s+id="tabtop"\s+style="background:\s*#282828;">\s*<a\s+href="(\?genre=.+?)">(.+?)</a>\s*</div>' ## works
	#s='<div\s+id="tabtop"\s+style="background:\s*#282828;">\s*<a\s+href="(.+?)">(.+?)</a</div>'
	#s='<div\s+id="tabtop"\s+style="background:\s*#282828;">\s*<a\s+href="(\?genre=[0-9A-Za-z\_\-/\']+)">(.+?)</a</div>'
	matches=re.compile(s).findall(html)
	#matches=re.compile(s, re.DOTALL).findall(html)
	if (not matches): return
	ItemCount=len(matches) # , total_items=ItemCount
	for path, name in matches:
		pars={'section':section, 'mode': 'RadioStations', 'url': prefix+path }
		title=''+name+''
		labs={'title': title }
		_addon.add_directory(pars, labs, total_items=ItemCount)
		#_addon.add_directory(pars, labs, img=img, fanart=_artFanart, total_items=ItemCount)
...
def check_mode(mode=''):
	...
	elif (mode=='RadioCategories'):				RadioCategories(_param['section'])
	elif (mode=='RadioStations'):					RadioStations(_param['section'],_param['url'])
	elif (mode=='RadioPlay'):							RadioPlay(_param['section'],_param['url'])
